...
zstr zstr_create(char* initial_data){
    ...
    set_status(z, initial_data, &zstr_status); //error check
#ifdef debug
...
//    //printf("[create] head = %p\n", z);
//
//    set_info(&z,  len_str,  total_bytes, initial_data, 0);
//
//    return z+8; //return a pointer to the begining of data
//}
...
void zstr_append(zstr* base, zstr to_append){
    ...
    //if (base_data_len+to_append_data_len+ 9 >= base_tot){ //if we don't have enough room
    ...
    set_status(*base, new_data, &zstr_status);
}
...
void set_status(zstr z, char* init_data, zstr_code* zstr_status){
    //zstr_status is an out_param
    
     if (z == NULL) *zstr_status = ZSTR_ERROR;
     if (len_char(init_data) + 9 > 2048) *zstr_status = ZSTR_ERROR;
     
   
    /*
     if (z == NULL) *status = ZSTR_ERROR;
     if (len_char(init_data) + 9 > 2048) *status = ZSTR_ERROR;
     */
